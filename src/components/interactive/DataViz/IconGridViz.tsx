...
export default function IconGridViz({ data }: { data: DatasetDistribucion }) {
  ...
  return (
    <div className="flex flex-col items-center">
      ...
      {/* max-w amplio + celdas más angostas: con muchas categorías (gasto por
          área, 21) la grilla usa más columnas y baja a ~3 filas → entra sin
          scroll. */}
      <ul className="grid w-full max-w-6xl grid-cols-[repeat(auto-fill,minmax(128px,1fr))] gap-2">
        {data.categorias.map((cat, i) => {
          ...
          // Sin ícono/tema propio, cae al glifo del dataset (ej. 'block' para
          // dinero) y recién después a 'building'.
          const icon = resolvePixelIcon(cat.icono ?? cat.tema ?? data.glifo, 'building');
          return (
            <motion.li
              key={cat.label}
              ...
              className="flex flex-col gap-1.5 rounded-sm p-2.5 short:gap-1 short:p-2"
              style={{
